...
const UPCOMING_AGENTS = {
  "code_interpreter": {
    ...
    desc: "A secure, offline sandbox for compiling and executing Python code snippets locally on CPU. Integrates strict sandbox boundaries and custom GBNF context grammar to block unauthorized system calls.",
    features: [
      "Local python execution using isolated subprocesses",
      "GBNF grammar validation to prevent subprocess, file system, or network access",
      "Direct variable state passing between agent turns",
      "Automatic error capture and correction feedback loop"
    ],
    code: "from slm_code_interpreter import SLMCodeInterpreter\n\nagent = SLMCodeInterpreter()\nresult = agent.execute(\"print('Hello World')\")\nprint(result) # Output: Hello World"
  },
  "git_copilot": {
    ...
    desc: "An offline git assistant that analyzes diffs and automatically generates detailed, structured commit messages adhering to Conventional Commits standards.",
    features: [
      "Automatic staged files diff retrieval",
      "Conventional Commits style parser (feat, fix, refactor, chore)",
      "Release notes and changelog markdown generator",
      "Completely local validation without network calls"
    ],
    code: "from slm_git_copilot import SLMGitCopilot\n\ncopilot = SLMGitCopilot()\nmessage = copilot.generate_commit_message()\nprint(message)"
  },
  "cli_agent": {
    name: "SLM CLI Agent",
    category: "Developer Tools",
    catClass: "badge-dev",
    stage: "Planning Phase",
    desc: "Offline shell helper that translates natural language commands to safe, platform-specific shell scripts and explains CLI flags before execution.",
    features: [
      "Natural language to bash/zsh/powershell translation",
      "Detailed flags and parameter breakdown explanation",
      "Dry-run safety validator to warn against destructive commands (rm -rf, etc.)",
      "Built-in command alias optimizer"
    ],
    code: "from slm_cli import SLMCLIAgent\n\nagent = SLMCLIAgent()\ncmd = agent.explain_and_build(\"find all files larger than 100MB\")\nprint(cmd)"
  },
  "json_cleaner": {
    ...
    catClass: "badge-dev",
    stage: "In Development",
    desc: "Correction agent specialized in auto-fixing truncated, malformed, or dirty JSON outputs generated by smaller-parameter language models.",
    features: [
      "Parser for incomplete bracket closures and missing quotes",
      "Regex-fallback JSON recovery for deep nested objects",
      "Strict schema validator matching target pydantic interfaces",
      "Zero-latency lightweight compilation on standard CPUs"
    ],
    code: "from slm_json_cleaner import SLMJsonCleaner\n\ncleaner = SLMJsonCleaner()\nclean_json = cleaner.fix(\"{\\\"name\\\": \\\"Test\\\", \\\"value\\\": 123\")\nprint(clean_json)"
  },
  "database_migrator": {
